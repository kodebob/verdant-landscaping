...
const PLAY_DURATION = 5500;
const FADE_DURATION = 1200;
const PRELOAD_AHEAD = 2500;
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-forest-950/55" style={{ zIndex: 3 }} />
      <div
      ...
      <div className="relative text-center px-6 max-w-5xl mx-auto" style={{ zIndex: 10 }}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a
            href={config.heroPrimaryButton.href}
          ...
          >
            {config.heroPrimaryButton.text}
          </a>
          <a
            href={config.heroSecondaryButton.href}
          ...
          >
            {config.heroSecondaryButton.text}
          </a>
        </div>
      </div>

      {/* Scroll indicator */}
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2" style={{ zIndex: 10 }}>
        <span className="text-white/45 text-[10px] tracking-[0.4em] uppercase font-sans">Scroll</span>
        <div className="w-px h-12 bg-gradient-to-b from-white/40 to-transparent" />
      </div>

      {/* Video progress dots */}
      <div className="absolute bottom-10 right-8 flex items-center gap-2" style={{ zIndex: 10 }}>
        {videos.map((_, i) => (
          <button
            key={i}
            onClick={() => jumpTo(i)}
